...
def pipeline_parallel_prefill(
    ...
    Each rank processes the same prompt chunks in the same order. Pipeline
    wrappers transfer the hidden state between ranks inside ``model()``: rank 0
    starts from token embeddings while downstream ranks receive the previous
    rank's output via ``PipelineFirstLayer``. This means all ranks must enter
    ``model()`` once per prompt chunk so the send/recv counts stay balanced.
    ...
    processed = 0
    logger.info(
        f"[R{rank}] Pipeline prefill: {n_real} synchronized real iterations"
    )
    clear_prefill_sends()
    ...
    try:
        with mx.stream(generation_stream):
            for i in range(n_real):
                ...
                if os.environ.get("EXO_DEBUG_PIPELINE", "").lower() in {
                    "1",
                    "true",
                    "yes",
                    "on",
                    ...
                    logger.info(
                        f"[R{rank}] prefill chunk {i + 1}/{n_real} model start tokens={chunk_size}"
                    )
                model(
                ...
                if os.environ.get("EXO_DEBUG_PIPELINE", "").lower() in {
                    ...
                    logger.info(
                        f"[R{rank}] prefill chunk {i + 1}/{n_real} model returned"
                    )
                ...
                if os.environ.get("EXO_DEBUG_PIPELINE", "").lower() in {
                    ...
                    logger.info(f"[R{rank}] prefill chunk {i + 1}/{n_real} flushed")
    ...
    logger.info(
        f"[R{rank}] Prefill: {n_real} synchronized real iterations, "
...
def prefill(
    ...
    try:
        # The custom overlapped prefill scheduler is only needed for prompts
        # large enough to split into multiple chunks. For a short prompt its
        # queued-send path can leave every rank waiting on the first Metal
        # graph. Use mlx_lm's synchronous stream_generate path instead.
