...
def _pipeline_debug_enabled() -> bool:
    return os.environ.get("EXO_DEBUG_PIPELINE", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def flush_prefill_sends() -> None:
    for output, dst, group in _pending_prefill_sends:
        if _pipeline_debug_enabled():
            logger.info(
                f"[R{group.rank()}] flushing queued prefill send to R{dst} shape={output.shape}"
            )
        sent = mx.distributed.send(output, dst, group=group)
        mx.async_eval(sent)
        if _pipeline_debug_enabled():
            logger.info(f"[R{group.rank()}] queued prefill send to R{dst} submitted")
...
class PipelineFirstLayer(CustomMlxLayer):
    ...
    def __call__(self, x: mx.array, *args: object, **kwargs: object) -> mx.array:
        if self.r != 0:
            ...
            if _pipeline_debug_enabled():
                logger.info(f"[R{self.r}] recv_like from R{self.r - 1} shape={x.shape}")
            mx.eval(x)
            x = mx.distributed.recv_like(x, (self.r - 1), group=self.group)
            mx.eval(x)
            if _pipeline_debug_enabled():
                logger.info(f"[R{self.r}] recv_like from R{self.r - 1} complete")
    ...
    def __call__(self, x: mx.array, *args: object, **kwargs: object) -> mx.array:
        ...
        if self.r != self.s - 1:
            if self.queue_sends:
                if _pipeline_debug_enabled():
                    logger.info(
                        f"[R{self.r}] queue prefill send to R{(self.r + 1) % self.s} shape={output.shape}"
                    )
