...
async def claude_request_to_text_generation(
    ...
    if request.thinking is not None:
        # Claude Code sends "adaptive" for its hosted Claude models. Local
        # Qwen-style thinking models can interpret that as an unbounded
        # reasoning pass, which makes even trivial Claude Code prompts stall.
        # Keep explicit Anthropic "enabled" as opt-in, but treat "adaptive" as
        # a no-thinking request for local compatibility.
        enable_thinking = request.thinking.type == "enabled"
