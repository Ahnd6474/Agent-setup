...
class TestClaudeRequestToInternal:
    ...
    async def test_request_accepts_system_message_role(self):
        request = ClaudeMessagesRequest(
            model=ModelId("claude-3-opus"),
            max_tokens=100,
            messages=[
        ...
        assert isinstance(params.input, list)
        assert [message.role for message in params.input] == ["system", "user"]
        assert params.chat_template_messages is not None
        assert params.chat_template_messages[0]["role"] == "system"

    async def test_adaptive_thinking_is_disabled_for_local_claude_adapter(self):
        request = ClaudeMessagesRequest(
            model=ModelId("claude-3-opus"),
            max_tokens=100,
            messages=[
    ...
    async def test_explicit_thinking_enabled_remains_opt_in(self):
        request = ClaudeMessagesRequest(
            model=ModelId("claude-3-opus"),
            max_tokens=100,
            messages=[
