...
class Master:
    ...
    async def _plan(self) -> None:
        while True:
            ...
            # time out dead nodes. Large MLX/Metal first-run compilation or
            # long prefill can delay node info events, so keep this configurable
            # instead of hard-coding a short 30s cluster timeout.
            node_timeout_seconds = float(os.environ.get("EXO_NODE_TIMEOUT_SECONDS", "30"))
            for node_id, time in self.state.last_seen.items():
                now = datetime.now(tz=timezone.utc)
                if now - time > timedelta(seconds=node_timeout_seconds):
