...
- node1의 `52415`에서 exo control plane/API가, `52416`에서 libp2p transport가
  동작한다. 2026-07-02 10:12 KST 재시도 후 topology에는 node1~node4 네 대가
  모두 보이고 `lastSeen`도 1초 이내로 갱신된다.
- node2와 node3는 Thunderbolt line SSH/API가 복구됐고 worker process가 실행 중이다.
- node4도 Thunderbolt line API가 복구됐고 worker process가 실행 중이다.
...
- Agentic Local Server는 llama.cpp `52417/v1`을 backend로 사용하며 실제 채팅이
  약 1.2초에 성공했다.
- exo `52415`에는 요청 단위 llama.cpp router 구현이 들어가 있고 replica 1개
  설정으로 비스트리밍과 스트리밍 Chat Completions 검증을 통과했다. 2026-07-02
  클러스터 재시작 후 live master는 `LLAMA_REPLICAS_FILE` 없이 떠 있어 현재 router
  status는 `enabled:false`다.
...
- Thunderbolt poll의 들여쓰기 결함을 수정하고 정상/timeout 경로 회귀 테스트를
  추가했다.
- 4노드 시작 스크립트는 worker별 line/net fallback, worker node-id 중복 검사와
  중복 keypair 회전을 수행하도록 갱신했다. 일부 worker가 죽어 있어도 살아있는
  worker까지 계속 시작한다. line preflight timeout도 실제 SSH timeout 설정을
  따르도록 수정해 링크 복구 직후의 느린 SSH handshake를 오판하지 않게 했다.
- thinking/reasoning streaming UI는 생성 중인 마지막 assistant 메시지에서 자동으로
  생각 패널을 펼치도록 수정했다.
- 갱신된 인증 DB, 실제 환경 설정, 로그와 세션을 암호화 runtime archive로 다시
  보존했고 복호화 및 SQLite 무결성 검사를 통과했다.

즉, node1 단독 사용자 서비스와 llama.cpp inference는 동작하고, 4노드
Thunderbolt/RDMA topology도 다시 live 상태다. 현재 가장 큰 미완료 항목은
multi-node MLX distributed 모델 배치/추론 경로와 dashboard type-check 정리다.
...
exo control plane :52415
  ...
  - matching model → llama replica router → :52417 (LLAMA_REPLICAS_FILE 설정 시)
...
현재 `screen`에는 `exo-master`, `exo-worker1`, `exo-worker2`, `exo-worker3`가 있다.

### 3.2 topology와 runner

현재 live state:

- topology node: 4 (`node1`, `node2`, `node3`, `node4`)
- connection group: 4
- runner: 0
- instance: 0
- Thunderbolt state node: 4
- `lastSeen`: 확인 시점 네 노드 모두 1초 이내
- RDMA edge: node1↔worker 방향의 Thunderbolt RDMA connection 확인
- state event index: 확인 시점 live 조회 기준
...
worker별로 management network로 fallback한다. 특정 worker가 timeout이어도 다른
worker 시작을 계속 진행한다. worker의 libp2p node id가 master와 같으면 기존
`~/.exo/node_id.keypair`를 백업하고 새 keypair를 생성해 topology에서 같은 노드로
합쳐지는 문제를 방지한다. 최소 노드 수가 이미 충족된 경우 기존 worker를
불필요하게 재시작하지 않는다.
