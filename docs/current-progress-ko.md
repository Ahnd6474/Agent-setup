...
- dashboard 정적 build는 성공하지만 `svelte-check`는 15 errors/6 warnings로
  실패한다.
- 현재 `info_gatherer.py`의 Thunderbolt poll 블록에는 timeout 조건 아래로
  수집 코드가 잘못 들어간 들여쓰기 결함이 있다.
...
- topology node: 4
- connection group: 4
- runner: 4개 모두 `RunnerRunning`
- state event index: 확인 시점 `16035`
...
마지막 항목은 일시적으로 느린 macOS system API가 전체 info gatherer task를
죽이지 않게 하기 위한 변경이다.
...
1. rank별 `EXO_DEBUG_PIPELINE` 로그의 마지막 send/recv 지점 비교
2. master task와 네 runner task ID 매핑
3. 짧은 prompt synchronous path에서 rank 0/1/2/3 진입 확인
4. Metal compile인지 distributed deadlock인지 구분
5. timeout 후 task cancel/runner 재사용 가능 여부 확인
...
### P1: Thunderbolt 주기 수집 들여쓰기 결함

`InfoGatherer._monitor_system_profiler_thunderbolt_data()`에서
`ThunderboltConnectivity.gather()`와 event 전송이
`if cancel_scope.cancel_called:` 내부에 들어가 있다. 정상적으로 30초 안에
interface map을 얻으면 조건이 false이므로 Thunderbolt identifier/connection
event를 보내지 않는다.

현재 topology/RDMA 직접 검증은 통과했지만 이는 기존 state와 다른 discovery
경로를 포함할 수 있어 이 poller의 정상 동작을 증명하지 않는다. 수집 코드를
timeout 조건 밖으로 옮기고 성공/timeout 두 경로를 검증하는 회귀 테스트가
필요하다.
...
4. Thunderbolt poll 들여쓰기 수정 및 회귀 테스트
5. dashboard 15개 type error 수정
6. OpenAI 1-token 성공 후 Claude/Ollama/Agentic 실제 생성 재검증
7. TTFT/tokens-per-second benchmark 기록
8. worker failure/rejoin 시험
9. 재부팅 자동 복구 구성 및 시험
10. Google OAuth 설정 후 callback 검증
