...
The API is used to manage model instances in the cluster, inspect cluster state,
and perform inference using multiple API-compatible interfaces. Port `52415`
belongs to the exo control plane: its browser UI is for monitoring and
administration, while its `/v1` endpoints are consumed by internal clients such
as the Agentic user application on port `8765`.

Port `8765` is not another exo API address. It is the separate end-user GUI and
application API described in [agentic-local-server.md](agentic-local-server.md).
